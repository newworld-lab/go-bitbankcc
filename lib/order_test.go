...
func TestPostCancelOrders(t *testing.T) {
	key := "key"
	secret := "secret"
	ctrl := gomock.NewController(t)
	client := NewMockclient(ctrl)
	...
	client.EXPECT().request(&clientOption{
		...
		header: http.Header{
			"ACCESS-KEY":       {"key"},
			"ACCESS-NONCE":     {"1531539009441"},
			"ACCESS-SIGNATURE": {"bc3839265c2a04c41a3d4c7b903cb1c92f5b7f4655c358d70c9efaf3ec15d3cb"},
		},
		body: []byte(`{"pair":"btc_jpy","order_ids":[444134158]}`),
	}).Return(
		[]byte(`{"success":1,"data":{"orders":[{"order_id":444134158,"pair":"btc_jpy","side":"buy","type":"limit","start_amount":"0.00100000","remaining_amount":"0.00100000","executed_amount":"0.00000000","price":"1000.0000","average_price":"0.0000","ordered_at":1533044000829,"status":"UNFILLED"}]}}`),
		nil,
	)
	...
	orders, err := api.PostCancelOrders(entity.PostCancelOrdersParams{
		Pair:     entity.PairBtcJpy,
		OrderIDs: []int{444134158},
	})
...
func TestPostOrdersInfo(t *testing.T) {
	key := "key"
	secret := "secret"
	ctrl := gomock.NewController(t)
	client := NewMockclient(ctrl)
	...
	orders, err := api.PostOrdersInfo(entity.PostOrdersInfoParams{
		Pair:     entity.PairBtcJpy,
		OrderIDs: []int{444134158},
	})
